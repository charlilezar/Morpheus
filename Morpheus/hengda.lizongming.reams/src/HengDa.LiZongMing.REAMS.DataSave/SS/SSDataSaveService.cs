...
namespace HengDa.LiZongMing.REAMS.DataSave.SS
{
    public class SSDataSaveService : ISingletonDependency
    {
        private readonly IServiceProvider _serviceProvider;
        private ILoggerFactory _loggerFactory;
        public ILogger Logger { get; set; }
        CtrlHDSSService _SSService = null;
        /// <summary>
        ...
        #endregion
        public SSDataSaveService(
             IServiceProvider serviceProvider,
            ILoggerFactory loggerFactory,
            CtrlHDSSService ssService)
        {
            this._SSService = ssService;
            this._serviceProvider = serviceProvider;
            this._loggerFactory = loggerFactory;
            this.Logger = loggerFactory.CreateLogger<SSDataSaveService>();
        }
        #region 数据采集过程
        HDSSCmdCode _HDSSCmdCode = HDSSCmdCode.瞬时数据查询;
        public int DynamicTaskDelay = 2000;
        byte[] _Datas = null;
        public void SetNextCmd(HDSSCmdCode cmd)
        {
            if (cmd == HDSSCmdCode.读取文件)
            {
                this._FileNo++;//如果下次还要读的话，则直接+1
                //如果是读取文件的，则一直读取，直至设备上的文件记录都被存入数据库为主；
                this._HDSSCmdCode = HDSSCmdCode.读取文件;
            ...
            else
            {
                ...
                switch (cmd)
                {
                    case HDSSCmdCode.瞬时数据查询:
                        this._HDSSCmdCode = HDSSCmdCode.故障查询;
                        break;
                    case HDSSCmdCode.故障查询:
                        this._HDSSCmdCode = HDSSCmdCode.设置信息查询;
                        break;
                    case HDSSCmdCode.设置信息查询:
                        this._HDSSCmdCode = HDSSCmdCode.工作状态查询;
                        break;
                    default:
                        this._HDSSCmdCode = HDSSCmdCode.瞬时数据查询;
                        break;
                }
            }
        }
        ...
        public void StartAsync(DeviceDto device)
        {
            ...
            * 3.1.5由于前几种方式同步的话，设备采集最好用同步等待的处理，现在统一异步不等待，所以逻辑上要稍微调整，就是文件读取发送之后
            ...
            //this._SSService = new CtrlHDSSService(this._loggerFactory, config);
            //this._SSService.Init(this._loggerFactory, config);
            //this._SSService.OnUniMessageReceivedCallBack = this.OnUniMessageReceivedCallBack;
            this._FileNo = 0;//初始化文件的读取序号
            this.SetNextCmd(HDSSCmdCode.读取文件);//每次开启先同步历史文件数据；
            AsyncHelper.RunSync(async () =>
            {
                while (true)
                {
                    Logger.LogWarning("SSDataSaveService working");
                    ...
                    if (!await this._SSService.TranService.TryOpenAsync())
                    {
                    ...
                    //开始采集
                    _SSService.SendTelecontrol();
                    if (this._HDSSCmdCode==HDSSCmdCode.读取文件)
                    {
                        await this._SSService.CmdExecWaitReply(this._HDSSCmdCode, this._Datas);
                    }
                    else
                    {
                        await this._SSService.CmdExec(this._HDSSCmdCode, this._Datas);
                    }
                    this.SetNextCmd(this._HDSSCmdCode);//执行下一个命令
        ...
        public void OnUniMessageReceivedCallBack(MqttClientConfig config, UniMessageBase uniMessageBase,SSBaseDto zCQBaseDto)
        {
            Logger.LogWarning("TODO SSBaseDto OnUniMessageReceivedCallBack");
