...
namespace Model.DataService
{
	...
	public class HveTcpService
	{
		...
		{
			HveTcpService.LocalIpAddress = null;
			...
			if (list.Count == 1)
			{
				HveTcpService.LocalIpAddress = list[0];
			}
			return list;
		}

		// Token: 0x060002E5 RID: 741 RVA: 0x0000761C File Offset: 0x0000581C
		private HveTcpService(Stream stream, string connectionName,/* Messenger mvvmMessenger, */bool isSerial)
		{
			...
			HveTcpService.ServiceDictionary.Add(connectionName, this);
		...
		public static Tuple<HveTcpService, Exception> StartSerialPortDataService(string commPortName, int baudRate, bool rtsControl)
		{
			Tuple<HveTcpService, Exception> result;
			try
			{
				...
				HveTcpService sentryDataService = new HveTcpService(baseStream, commPortName, /*(Messenger)Messenger.Default,*/ true)
				{
				...
				result = new Tuple<HveTcpService, Exception>(sentryDataService, null);
			}
			catch (Exception)
			{
				result = new Tuple<HveTcpService, Exception>(null, new ApplicationException("Unable to open serial port " + commPortName));
			}
			return result;
		}
		...
		/// <returns></returns>
		public static Tuple<HveTcpService, Exception> StartSocketDataService(string ipAddress, int port)
		{
			Tuple<HveTcpService, Exception> result;
			try
			{
				...
				{
					ipAddress = HveTcpService.LocalIpAddress.ToString();
				}
				IPAddress address = IPAddress.Parse(ipAddress);
				IPEndPoint remoteEP = new IPEndPoint(address, port);
				socket.Connect(remoteEP);
				Stream stream = new NetworkStream(socket);
				HveTcpService sentryDataService = new HveTcpService(stream, ipAddress, /*(Messenger)Messenger.Default,*/ false)
				{
			...
			{
				result = new Tuple<HveTcpService, Exception>(null, item);
			}
			return result;
		}
		...
		public void CloseDataService()
		{
			...
			HveTcpService.ServiceDictionary.Remove(this.ConnectionName);
			if (this._stream != null)
			{
				this._stream.Close();
			}
		...
		private bool FindSyncByte(int bytesReadFromBuffer)
		{
			for (int i = this._receiveBufferPointer; i < bytesReadFromBuffer; i++)
			{
				if (this._receiveBuffer[i] == 240)
				{
					...
					this._responseState = HveTcpService.ReadResponseState.WaitingForByteCount;
		...
		private bool ReadResponseByteCount(int readBufferLength, ref int bytesRead)
		{
			...
			{
				this._responseState = HveTcpService.ReadResponseState.WaitingForSync;
				return true;
			}
			int num3 = bytesRead;
			SentryDebug.WriteLineConditional("Byte count=" + bytesRead.ToString());
			this._responseState = HveTcpService.ReadResponseState.ReadingResponse;
		...
		public void ReceiveCallback(IAsyncResult ar)
		{
			try
			{
				...
				while (flag)
				{
					if (this._responseState == HveTcpService.ReadResponseState.WaitingForSync)
					{
						flag = this.FindSyncByte(num);
					}
					else if (this._responseState == HveTcpService.ReadResponseState.WaitingForByteCount)
					{
		...
		public void ProcessResponse(byte[] buffer, int length)
		{
			...
			Tuple<object, Type> tuple = HveTcpService.DeserializeUnknownCommand(@string);
		...
		public static Dictionary<string, HveTcpService> ServiceDictionary = new Dictionary<string, HveTcpService>();
		...
		private HveTcpService.ReadResponseState _responseState;
