		...
		private static void ReceiveCallback(IAsyncResult ar)
		{
			if (!UdpNotifyInput.AppClosing)
			{
				try
				{
					...
					if (SentryDataService.DebugXml)
					{
						Logger.Trace($"  ===UDP接收到 UdpNotify : lenght:{num2}\r\n```\r\n" + @string + "\r\n```\r\n");
					}


					if (SentryDataService.ServiceDictionary.ContainsKey(ipendPoint.Address.ToString()))
					{
						SentryDataService dataService = SentryDataService.ServiceDictionary[ipendPoint.Address.ToString()];
						...
						if (stringBetween == "ScalarMeasurement") //标量测量值
						{
							...
							if (scalarMeasurement != null)
							{
								mvvmScalarMeasurement message = new mvvmScalarMeasurement(dataService, scalarMeasurement);
								UdpNotifyInput.s_MvvmMessenger.Send<mvvmScalarMeasurement>(message);
							}
							else
							{
						...
						else if (stringBetween == "SentinelState") //哨兵状态
						{
							SentinelState sentinelState = SentinelState.DeserializeFromStream(stream);
							if (sentinelState != null)
							{
								mvvmSentinelState message2 = new mvvmSentinelState(dataService, sentinelState);
								UdpNotifyInput.s_MvvmMessenger.Send<mvvmSentinelState>(message2);
							}
							else
							{
						...
						else if (stringBetween == "WaveformMeasurement")//波形计量
						{
							...
							if (waveformMeasurement != null)
							{
								mvvmWaveformMeasurement message3 = new mvvmWaveformMeasurement(dataService, waveformMeasurement);
								UdpNotifyInput.s_MvvmMessenger.Send<mvvmWaveformMeasurement>(message3);
							}
						}
						else if (stringBetween == "BatteryMeasurement") //电池计量
						{
							...
							{
								mvvmBatteryMeasurement message4 = new mvvmBatteryMeasurement(dataService, batteryMeasurement);
								UdpNotifyInput.s_MvvmMessenger.Send<mvvmBatteryMeasurement>(message4);
							}
						}
					}
				}catch(Exception ex)
